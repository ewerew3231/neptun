...
OPENCAGE_API_KEY = '30263924aa374a45a8b1b0469cb8d347'
...
def advanced_ner_locations(text):
    """
    Распознаёт локации с помощью HuggingFace NER (украинский/русский), spaCy и fallback-логики.
    ...
    if hf_ner:
        try:
            results = hf_ner(text)
            for ent in results:
                if ent['entity_group'] in ("LOC", "GPE") and len(ent['word']) > 2:
                    loc = ent['word'].strip()
                    locations[loc] = {'type': ent['entity_group'], 'context': 'hf_ner'}
        except Exception as e:
    ...
    if nlp_uk:
        doc = nlp_uk(text)
        for ent in doc.ents:
            if ent.label_ in ("LOC", "GPE", "FAC") and len(ent.text) > 2:
                loc = ent.text.strip()
                if loc not in locations:
                    locations[loc] = {'type': ent.label_, 'context': 'spacy'}
...
def should_ignore_message(text):
    ...
    Возвращает True если сообщение нужно игнорировать.
    """
    # 1. Проверяем наличие конкретного населенного пункта
    location_pattern = r'(?:[А-ЯІЇЄҐ][а-яіїєґ\'\-]+(?:\s+[А-ЯІЇЄҐ][а-яіїєґ\'\-]+)*)\s*(?:район|місто|село|смт|селище|мікрорайон)?'
    location_with_oblast = r'([А-ЯІЇЄҐ][А-ЯІЇЄҐа-яіїєґ\'\-\s]+)\s*\(([А-ЯІЇЄҐа-яіїєґ\'\-\s]+)\s*(обл|область)\.?\)'
    
    has_specific_location = bool(re.search(location_pattern, text)) or bool(re.search(location_with_oblast, text))
    ...
    vague_location_words = [
        ...
        'локаційно', 'территори', 'територі',
        'дорозвідка', 'разведка', 'розвідка'
    ]
    
    # 2. Игнорируем сообщения только с общими указаниями
    found_location = None
    if has_specific_location:
        # Извлекаем конкретное место из текста
        match = re.search(location_pattern, text)
        if match:
            found_location = match.group(0)
        match = re.search(location_with_oblast, text)
        if match:
            found_location = match.group(1)
    
    # Если найденное место является общим указанием - игнорируем
    if found_location:
        location_lower = found_location.lower()
        for word in vague_location_words:
            if re.search(word, location_lower):
                return True
    
    # 3. Если в сообщении нет конкретного места - игнорируем
...
def process_message(message_text, message_id, date, channel):
    ...
    # --- Если найдено две и более локации, формируем lat2/lng2 для шахедов и ракет ---
    loc_items = list(locations.items())
    if len(loc_items) >= 2:
        # Первая точка
        place1, meta1 = loc_items[0]
        place2, meta2 = loc_items[1]
        place_norm1 = normalize_place_name(place1)
        place_norm2 = normalize_place_name(place2)
        # Координаты первой точки
        if re.search(r'(ська|ская|область|region)$', place_norm1, re.IGNORECASE):
            oblast_key = place_norm1.replace(' область', '').replace('ская', 'ская').replace('ська', 'ська').strip()
            coords1 = OBLAST_CENTERS.get(oblast_key) or geocode_place_opencage(place1)
        elif place_norm1 in PRIORITY_LOCATIONS:
            coords1 = PRIORITY_LOCATIONS[place_norm1]
        else:
            coords1 = geocode_place_opencage(place1)
        # Координаты второй точки
        if re.search(r'(ська|ская|область|region)$', place_norm2, re.IGNORECASE):
            oblast_key2 = place_norm2.replace(' область', '').replace('ская', 'ская').replace('ська', 'ська').strip()
            coords2 = OBLAST_CENTERS.get(oblast_key2) or geocode_place_opencage(place2)
        elif place_norm2 in PRIORITY_LOCATIONS:
            coords2 = PRIORITY_LOCATIONS[place_norm2]
        else:
            coords2 = geocode_place_opencage(place2)
        if coords1 and coords2:
            lat1, lng1 = coords1
            lat2, lng2 = coords2
            threat_type = get_threat_type(message_text)
            if threat_type in ['shahed', 'raketa']:
                if not (44.0 <= lat1 <= 53.0 and 22.0 <= lng1 <= 41.0):
                    return None
                marker_icon = 'shahed.png' if threat_type == 'shahed' else 'raketa.png'
                track = {
                    'id': message_id,
                    'channel': channel,
                    'date': date,
                ...
                return [track]
    # --- Обычная логика: один маркер ---
    added_city = False
    for place, meta in locations.items():
        place_norm = normalize_place_name(place)
        # Если это область — используем центр области (всегда, даже для "Полтавщина")
        # Улучшенная нормализация для областей
        oblast_key = None
        norm_place = place_norm.lower().replace(' область', '').replace(' обл.', '').replace('ская', 'ская').replace('ська', 'ська').strip()
        for key in OBLAST_CENTERS:
            norm_key = key.lower().replace(' область', '').replace(' обл.', '').replace('ская', 'ская').replace('ська', 'ська').strip()
            if norm_place == norm_key or norm_place in norm_key or norm_key in norm_place:
                oblast_key = key
                break
        if oblast_key:
            coords = OBLAST_CENTERS[oblast_key]
            lat, lng = coords
        elif re.search(r'(ська|ская|область|region)$', place_norm, re.IGNORECASE):
            coords = geocode_place_opencage(place)
            if not coords:
                continue
            lat, lng = coords
        # Для городов — PRIORITY_LOCATIONS, с учетом области если есть
        oblast_hint = None
        oblast_match = re.search(r'\(([^\)]+ обл\.?|область)\)', message_text)
        if oblast_match:
            oblast_hint = oblast_match.group(1).replace(' обл.', '').replace(' область', '').strip().lower()
        # Если есть область, ищем город с учетом области
        found_city = False
        if oblast_hint:
            for city_key in PRIORITY_LOCATIONS:
                if place_norm == city_key:
                    # Проверяем, есть ли область в meta или message_text
                    # Если город уникален — берем
                    lat_c, lng_c = PRIORITY_LOCATIONS[city_key]
                    # Проверка по области: если координаты города близки к центру нужной области
                    for oblast_key in OBLAST_CENTERS:
                        if oblast_hint in oblast_key:
                            lat_o, lng_o = OBLAST_CENTERS[oblast_key]
                            dist = math.sqrt((lat_c-lat_o)**2 + (lng_c-lng_o)**2)
                            if dist < 2.5:  # менее 2.5 градуса — вероятно, тот самый город
                                lat, lng = lat_c, lng_c
                                found_city = True
                                break
                    if found_city:
                        break
        if not found_city and place_norm in PRIORITY_LOCATIONS:
            lat, lng = PRIORITY_LOCATIONS[place_norm]
        else:
            coords = geocode_place_opencage(place)
            if not coords:
                continue
        ...
        if key in unique_cities:
            continue
        # Only add first city-level marker
        if not added_city:
            unique_cities.add(key)
            if not (44.0 <= lat <= 53.0 and 22.0 <= lng <= 41.0):
                logger.warning(f"Пропущена метка вне Украины/моря: {place} ({lat}, {lng})")
                continue
            direction = meta.get('direction')
            threat_type = get_threat_type(message_text)
            # Marker icon по типу угрозы
            if threat_type == 'recon_uav':
                marker_icon = 'rozved.png'
            elif threat_type == 'raketa':
                marker_icon = 'raketa.png'
            elif threat_type == 'avia':
                marker_icon = 'avia.png'
            else:
            ...
            if threat_type == 'shahed':
                if direction:
                    try:
                        ...
                        angle = 0
                    dist_km = 25
                    dlat = dist_km / 111 * math.cos(math.radians(angle))
                    dlng = dist_km / (111 * math.cos(math.radians(lat))) * math.sin(math.radians(angle))
                    lat2 = lat + dlat
                    lng2 = lng + dlng
                else:
                    # Если нет направления и только одна точка — двигаем на запад
                    lat2 = lat
                    lng2 = lng - 0.5
                track = {
                    'id': message_id,
            ...
            tracks.append(track)
            added_city = True
    # If no city found, fallback to first valid location
    if not tracks:
        for place, meta in locations.items():
            place_norm = normalize_place_name(place)
            coords = geocode_place_opencage(place)
...
from export_utils import export_tracks_to_csv
